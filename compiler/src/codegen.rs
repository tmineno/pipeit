...
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            errors
                .iter()
                .any(|d| d.code.as_ref().map(|c| c.0) == Some("E0700")),
            "shm transport should produce E0700 diagnostic, got: {:?}",
            errors
        );
        assert!(
            !result
...
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            warnings
                .iter()
                .any(|d| d.code.as_ref().map(|c| c.0) == Some("W0700")),
            "udp() with no positional arg should produce W0700 warning, got: {:?}",
            warnings
        );
    }
