...
impl<'a> CodegenCtx<'a> {
    ...
    fn emit_bind_storage(&mut self) {
        ...
        // _apply_pending_rebinds() is emitted in emit_bind_io_adapters()
        // (after we know which binds are lowered, for reconnect wiring).
        self.out.push('\n');
    }

    fn effective_endpoint(&self, bind: &LirBind) -> String {
        self.options
            .bind_overrides
            .get(&bind.name)
            .cloned()
            .unwrap_or_else(|| self.format_resolved_endpoint_spec(bind))
    }

    /// Format full spec string for BindDesc display (best-effort, no diagnostic).
    fn format_resolved_endpoint_spec(&self, bind: &LirBind) -> String {
        let mut s = format!("{}(", bind.transport);
        for (i, arg) in bind.args.iter().enumerate() {
            if i > 0 {
                s.push_str(", ");
            }
            match arg {
                LirBindArg::Positional(LirBindValue::Ident(name)) => {
                    if let Some(val) = self.resolve_const_literal(name) {
                        s.push_str(&val);
                    } else {
                        s.push_str(name);
                    }
    ...
    /// Look up a const name, return its C++ literal (with quotes for strings).
    fn resolve_const_literal(&self, name: &str) -> Option<String> {
        self.lir.consts.iter().find_map(|c| {
            if c.name == name {
                if let LirConstValue::Scalar { literal } = &c.value {
                    Some(literal.clone())
                } else {
                    None
                }
    ...
    /// Map PipitType to PPKT DType C++ constant name.
    fn pipit_type_to_ppkt_dtype(dtype: &PipitType) -> Option<&'static str> {
        match dtype {
            PipitType::Float => Some("pipit::net::DTYPE_F32"),
            PipitType::Int32 => Some("pipit::net::DTYPE_I32"),
            PipitType::Cfloat => Some("pipit::net::DTYPE_CF32"),
    ...
    /// Extract chan_id from named "chan" argument, default 0.
    fn bind_chan_id(bind: &LirBind) -> u16 {
        for arg in &bind.args {
            if let LirBindArg::Named(k, LirBindValue::Int(n)) = arg {
                if k == "chan" {
                    return *n as u16;
                }
            }
    ...
    fn emit_bind_io_adapters(&mut self) {
        ...
        for bind in &self.lir.binds {
            let contract = match &bind.contract {
                Some(c) => c,
                None => continue,
            };
                ...
                other => {
                    diags.push(
                        Diagnostic::new(
                            ...
                            codegen_span(),
                            format!(
                                "bind '{}': transport '{}' is not yet supported for I/O lowering",
                                bind.name, other
                            ),
                        )
                        .with_code(codes::E0710),
                    );
                    continue;
                }
            }
                    ...
                    None => {
                        diags.push(
                            Diagnostic::new(
                                DiagLevel::Error,
                                codegen_span(),
                                format!("bind '{}': dtype '{}' has no PPKT mapping", bind.name, dt),
                            )
                            .with_code(codes::E0711),
                        );
                        continue;
                    }
                },
                None => {
                    diags.push(
                        Diagnostic::new(
                            ...
                            codegen_span(),
                            format!(
                                "bind '{}': dtype could not be inferred; no I/O adapter emitted",
                                bind.name
                            ),
                        )
                        .with_code(codes::W0711),
                    );
                    continue;
                }
            };

            // Endpoint guard — check if positional arg is resolvable
            let mut found_positional = false;
            let mut resolvable = true;
            for arg in &bind.args {
                if let LirBindArg::Positional(val) = arg {
                    found_positional = true;
                    if let LirBindValue::Ident(name) = val {
                        let found = self.lir.consts.iter().any(|c| {
                            c.name == *name && matches!(&c.value, LirConstValue::Scalar { .. })
                        });
                        if !found {
                            diags.push(
                                Diagnostic::new(
                                    ...
                                    codegen_span(),
                                    format!(
                                        "bind '{}': endpoint argument '{}' is not a known const",
                                        bind.name, name
                                    ),
                                )
                                .with_code(codes::E0712),
                            );
                            resolvable = false;
                        }
                    }
            ...
            if !found_positional {
                diags.push(
                    Diagnostic::new(
                        ...
                        codegen_span(),
                        format!(
                            "bind '{}': no endpoint address; use --bind or rebind() to activate",
                            bind.name
                        ),
                    )
                    .with_code(codes::W0710),
                );
            }
            if !resolvable {
                continue;
    ...
    fn emit_apply_pending_rebinds(&mut self) {
        self.out
        ...
        match &firing.kind {
            ...
            LirFiringKind::BufferRead(io) => {
                let use_recv = self
                    ...
                    .binds
                    .iter()
                    .find(|b| b.name == io.buffer_name)
                    .and_then(|b| {
                        let is_in =
                            b.contract.as_ref().map(|c| c.direction) == Some(BindDirection::In);
                        if is_in && self.lowered_binds.contains(&b.name) {
                            Some(())
                        } else {
                            None
                        }
            ...
            LirFiringKind::BufferWrite(io) => {
                let use_send = self
                    ...
                    .and_then(|b| {
                        let is_out =
                            b.contract.as_ref().map(|c| c.direction) == Some(BindDirection::Out);
                        if is_out && self.lowered_binds.contains(&b.name) {
                            Some(())
                        } else {
                            None
                        }
            ...
            LirFiringKind::GatherRead(io) => {
                self.emit_lir_gather_read(task_name, io, ind);
            }
            LirFiringKind::ScatterWrite(io) => {
                self.emit_lir_scatter_write(task_name, io, ind);
            }
        }
    ...
    /// Emit gather read: N sequential spin-wait reads from element ring buffers
    /// into a contiguous output edge variable.
    fn emit_lir_gather_read(&mut self, task_name: &str, io: &LirGatherIo, indent: &str) {
        let timeout_ms = self.lir.directives.wait_timeout_ms;
        let _ = writeln!(
            self.out,
            "{}// gather @{}[*] — {} elements",
            indent, io.family_name, io.element_count
        );
        for elem in &io.elements {
            ...
            let wr_var = format!("_wr_{}_{}", elem.src_node_id.0, elem.peer_node_id.0);
            let _ = writeln!(
                self.out,
                "{}    auto {} = _ringbuf_{}.wait_readable({}, {}, _stop, std::chrono::milliseconds({}));",
                indent, wr_var, elem.buffer_name, elem.reader_idx, elem.tokens, timeout_ms
            );
            let _ = writeln!(
                self.out,
                "{}    if ({} == pipit::WaitResult::stopped) return;",
            ...
            let _ = writeln!(
                self.out,
                "{}        std::fprintf(stderr, \"runtime error: task '{}' timeout waiting for {} token(s) from shared buffer '{}'\\n\");",
                indent, task_name, elem.tokens, elem.buffer_name
            );
            let _ = writeln!(
                self.out,
                "{}        _exit_code.store(1, std::memory_order_release);",
    ...
    /// Emit scatter write: N sequential spin-wait writes of slices from a contiguous
    /// input edge variable to element ring buffers.
    fn emit_lir_scatter_write(&mut self, task_name: &str, io: &LirScatterIo, indent: &str) {
        let timeout_ms = self.lir.directives.wait_timeout_ms;
        let _ = writeln!(
            self.out,
            "{}// scatter ->{}[*] — {} elements",
        ...
        for elem in &io.elements {
            ...
            let ww_var = format!("_ww_{}_{}", elem.src_node_id.0, elem.peer_node_id.0);
            let _ = writeln!(
                self.out,
                "{}    auto {} = _ringbuf_{}.wait_writable({}, _stop, std::chrono::milliseconds({}));",
                indent, ww_var, elem.buffer_name, elem.tokens, timeout_ms
            );
            let _ = writeln!(
                self.out,
                "{}    if ({} == pipit::WaitResult::stopped) return;",
...
fn codegen_span() -> crate::ast::Span {
...
mod tests {
    ...
    // ── Bind I/O adapter codegen tests ──────────────────────────────────

    #[test]
    fn bind_out_emits_adapter_instance() {
        let reg = test_registry();
        let cpp = codegen_ok(
...
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            cpp.contains("pipit::BindIoAdapter _bind_io_iq("),
            "should emit BindIoAdapter instance for OUT bind, got:\n{}",
            cpp
        );
        assert!(
            cpp.contains("true,"),
            "OUT bind adapter should have is_out=true"
        );
    }

    #[test]
    fn bind_out_emits_send_call() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=10)
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            cpp.contains("_bind_io_iq.send("),
            "OUT bind should emit send() call at buf_write, got:\n{}",
            cpp
        );
    }

    #[test]
    fn bind_in_emits_recv_call() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=10)
clock 48kHz audio {
    @iq | binwrite("/dev/null")
        ...
        assert!(
            ...
            "IN bind should emit recv() call at buf_read, got:\n{}",
            cpp
        );
        assert!(
            cpp.contains("false,"),
            "IN bind adapter should have is_out=false"
        );
    }

    #[test]
    fn bind_out_skip_writes_still_sends() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=10)
clock 48kHz audio {
    constant(0) -> iq
        ...
        // OUT bind with no local readers has skip=true, but send() should still be emitted
        assert!(
            cpp.contains("_bind_io_iq.send("),
            "OUT bind with skip=true should still emit send(), got:\n{}",
            cpp
        );
    }

    #[test]
    fn bind_rebind_emits_reconnect() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=10)
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            cpp.contains("_bind_io_iq.reconnect("),
            "_apply_pending_rebinds should contain reconnect() call, got:\n{}",
            cpp
        );
    }

    #[test]
    fn bind_rebind_reconnect_after_unlock() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=10)
clock 48kHz audio {
    constant(0) -> iq
        ...
        // The reconnect call should appear after the closing brace of the lock_guard scope
        let rebind_fn = cpp
            .find("_apply_pending_rebinds")
            .expect("should contain _apply_pending_rebinds");
        let rebind_section = &cpp[rebind_fn..];
        let lock_end = rebind_section.find("_did_rebind_iq = true;").unwrap();
        let reconnect_pos = rebind_section.find("_bind_io_iq.reconnect(").unwrap();
        assert!(
            reconnect_pos > lock_end,
            "reconnect() should be called after lock_guard scope ends"
        );
    }

    #[test]
    fn bind_preamble_includes_bind_io() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=10)
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            ...
            "programs with binds should include pipit_bind_io.h, got:\n{}",
            cpp
        );
    }

    #[test]
    fn no_bind_preamble_unchanged() {
        let reg = test_registry();
        let cpp = codegen_ok("clock 1kHz t { constant(0.0) | stdout() }", &reg);
        assert!(
            !cpp.contains("#include <pipit_bind_io.h>"),
    ...
    #[test]
    fn bind_ident_endpoint_resolved() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"const addr = "127.0.0.1:9100"
...
clock 48kHz audio {
    constant(0) -> iq
        ...
        // BindState should contain resolved spec string with C++ escaped quotes
        assert!(
            cpp.contains(r#"udp(\"127.0.0.1:9100\""#),
            "BindState endpoint should resolve Ident to const value, got:\n{}",
            cpp
        );
        assert!(
            !cpp.contains(r#"udp(addr,"#),
            "BindState endpoint should not contain unresolved Ident name"
        );
    }
...
clock 48kHz audio {
    constant(0) -> iq
        ...
        assert!(
            warnings
                .iter()
                .any(|d| d.code.as_ref().map(|c| c.0) == Some("W0710")),
            "udp() with no positional arg should produce W0710 warning, got: {:?}",
            warnings
        );
    }

    #[test]
    fn bind_adapter_chan_id() {
        let reg = test_registry();
        let cpp = codegen_ok(
            r#"bind iq = udp("127.0.0.1:9100", chan=42)
clock 48kHz audio {
    constant(0) -> iq
}"#,
            &reg,
        ...
        assert!(
            cpp.contains(", 42,"),
            "adapter should have chan_id=42 from bind args, got:\n{}",
            cpp
        );
    }
}
