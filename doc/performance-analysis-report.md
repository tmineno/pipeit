...
## 8. Addendum: Runtime Benchmark Regression Check (`main` vs `review/spec`)

Date: 2026-02-18  
Compared revisions:

- `main`: `a3bfdb9e56843e50153433cc231b49c321c3c3d1`
- `review/spec`: `f8e2fe08c849d68b6b9d6af6525626faef714f58`

Host/environment:

- Same host as §1 (WSL2, Linux `6.6.87.2-microsoft-standard-WSL2`)
- `ringbuf/timer/thread` categories measured twice per branch, then averaged by benchmark name.

Commands:

```bash
bash benches/run_all.sh --filter ringbuf --filter timer --filter thread --output-dir /tmp/pipit-runtime-current
bash benches/run_all.sh --filter ringbuf --filter timer --filter thread --output-dir /tmp/pipit-runtime-current-2

# main worktree at /tmp/pipit-main-bench
bash benches/run_all.sh --filter ringbuf --filter timer --filter thread --output-dir /tmp/pipit-runtime-main
bash benches/run_all.sh --filter ringbuf --filter timer --filter thread --output-dir /tmp/pipit-runtime-main-2
```

### 8.1 Category-level summary (CPU time, lower is better)

| Category | Bench Count | Mean Delta (`review/spec` vs `main`) | Regressions >5% | Regressions >10% |
|---|---:|---:|---:|---:|
| ringbuf | 12 | **-0.75%** | 1 | 0 |
| timer | 19 | **-1.36%** | 1 | 0 |
| thread | 14 | **-2.35%** | 1 | 1 |
...
- No systemic runtime degradation was observed in aggregate for `ringbuf`, `timer`, or `thread`.
- All three runtime categories improved on average vs `main`.

### 8.2 Largest regressions and improvements (averaged runs)

Largest regressions:

- `BM_TaskScaling/2`: `+11.99%`
- `BM_Timer_HighFreqBatched/10000000/100/2000`: `+7.13%`
- `BM_RingBuffer_ChunkScaling/256`: `+5.58%`

Largest improvements:

- `BM_TaskScaling/4`: `-28.74%`
- `BM_RingBuffer_Contention/2readers`: `-15.05%`
- `BM_Timer_FrequencySweep/1000000/1000`: `-9.85%`

### 8.3 PDL benchmark caveat for branch-to-branch comparison

`pdl` category is **not** a clean apples-to-apples regression baseline between these two revisions:

- `main` frequently terminates early in modal/shared-buffer cases (example observed: `runtime error: task 'adaptive' failed to write 1 token(s) to shared buffer 'ctrl'`), producing truncated tick counts.
- `review/spec` includes behavioral fixes that avoid this premature stop in the same scenarios.

Therefore, this regression check uses `ringbuf/timer/thread` as the stable runtime KPI baseline for branch comparison.
