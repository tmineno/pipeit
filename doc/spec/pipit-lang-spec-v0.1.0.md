# Pipit Language Specification v0.1.0
...
Pipit のソースファイル (`.pdl`: Pipit Definition Language) は専用コンパイラ `pcc` (Pipit Compiler Collection) によって C++ コードに変換され、標準 C++ コンパイラでビルドされる。

```
source.pdl → pcc → source_gen.cpp → g++/clang++ → executable
```

アクター定義は C++ で記述し、`ACTOR` マクロが `constexpr` 登録関数を生成する。`pcc` はこの登録情報を参照して型・レート情報を取得する（C++ パーサーは不要）。生成コードは Pipit ランタイムライブラリ `libpipit` とリンクする。
...
source.pdl + actors.h
  ...
  │
  ├─ 2. アクター登録情報の読込み
  │     └─ ACTOR マクロが生成した constexpr 登録関数から
  │        型・レート・パラメータ情報を取得
  │
  ├─ 3. 名前解決
  │     ├─ アクター名: 登録情報から検索（括弧付きのみ）
  │     ├─ 共有バッファ名: -> で定義、@ で参照
  │     ├─ タップ名: : で定義・参照（タスクスコープ）
  │     ├─ const / param 名: グローバルスコープ
  │     └─ 名前の衝突検出（同一名前空間内の重複禁止）
  │
  ├─ 4. SDF グラフ構築
  │     ├─ タップをフォークノードに展開
  │     ├─ define をインライン展開
  │     ├─ 共有バッファをタスク間エッジに変換
  │     ├─ control subgraph を独立サブグラフとして構築
  │     └─ 各 mode ブロックを独立サブグラフとして構築
  │
  ├─ 5. 静的解析
  │     ├─ 型整合性検証
  │     ├─ SDF バランス方程式の求解 → repetition vector 算出
  │     ├─ フィードバックループの delay 検証
  │     ├─ クロックドメイン境界のレート整合性検証 (Pw×fw = Cr×fr)
  │     ├─ CSDF モードごとの個別解析
  │     ├─ ctrl 供給元の検証
  │     └─ バッファサイズの算出（安全側上界）
  │
  ...
  │     ├─ 各タスク内の PASS 構築
  │     ├─ K (イテレーション / ティック) の決定
  │     └─ バッチ化最適化（高 target rate 対応）
  │
  ├─ 7. C++ コード生成
  │     ├─ リングバッファの静的配置コード
  │     ├─ タスクごとのスケジュールループ
  │     ├─ ランタイムパラメータのダブルバッファリング機構
  │     ├─ オーバーラン検出・統計収集コード
  │     ├─ プローブ出力コード（--release 時は除去）
  │     └─ main() 関数（CLI 引数パーサーを含む）
  │
  └─ 8. C++ コンパイル
        └─ g++/clang++ -O2 -lpipit → 実行形式
```
...
以下の機能は v0.1.0 では対象外とするが、将来の拡張候補として記録する。
